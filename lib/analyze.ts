...
4. 一个英文例句
5. 例句的中文翻译（不可以包含英文）
6. 3个用于选择题的中文干扰项（必须包含词性，例如 "adj. 快的"）。注意：中文干扰项必须具有迷惑性，且选项绝对不可以出现英文，选项之间不可以出现重复的词语（除非词性不同，例如如果一个单词的意思是adj.潜在的；可能的，那么干扰项就不可以单独出现一个adj. 可能的；function的意思是n. 功能；职务 v. 运转；起作用，但他也有n. 函数，你在翻译时没给出n. 函数，那么选项中就不能出现类似于n. 函数；v. 起作用的选项，因为这个选项也是正确的）。**非常重要：如果正确答案（简短中文翻译）包含多个词性或多个意思导致较长，请确保这3个干扰项也具有类似的长度和格式（例如也包含多个词性和意思），避免用户通过选项长度直接猜出正确答案。**
7. 3个用于选择题的英文干扰项（形近词或其他单词，必须是三个，且与题目和干扰项之间不能相同，例如，题目中或一个干扰项为principal，那么其他干扰项就不能是principal）
